...
nominal_capacity_wh = 5000  # in Wh
charge_eff = 0.96
discharge_eff = 0.96
initial_soc_wh = 0.5 * nominal_capacity_wh
discharge_c_rate = 0.77
nominal_voltage_dc = 76.8  # Nominal DC voltage in Volts
...
pdf_path = os.path.join("pdf", "soc_jun50k23.pdf") 
...
with PdfPages(pdf_path) as pdf: 
    ...
    while current_date <= end_date:
        ...
        if not df_filtered.empty:
            ...
            fig, axes = plt.subplots(nrows=3, ncols=1, figsize=(12, 15), sharex=True) # Increased nrows to 3
            plt.subplots_adjust(hspace=0.5)

            #Demand and PV Distribution
            axes[0].plot(df_filtered.index, df_filtered["Demand2"].fillna(0), label="Demand (W)", color="red")
            axes[0].plot(df_filtered.index, df_filtered["PV_distribution"].fillna(0), label="PV Distribution (W)", color="lightgreen")
