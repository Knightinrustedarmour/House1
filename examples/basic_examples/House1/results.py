...
actual_demand = op_data["Load_W"].sum() / 1000
actual_pv_production = op_data["Modelled_Energy"].sum() / 1000

print("Total PV Production (Actual): {:.2f} kW".format(actual_pv_production))
print("Total Demand (Actual): {:.2f} kW".format(actual_demand))
...
# df["PV_distribution"].plot(ax=axes[0], label="PV Distribution (Wh)", color="lightgreen")
# axes[0].set_ylabel("Power (W)")
# axes[0].legend(bbox_to_anchor=(1, 1), loc='upper right')
# axes[0].grid(True)


# df["Demand2"].plot(ax=axes[1], label="Demand (Wh)", color="red")
# df["Grid_import"].plot(ax=axes[1], label="Grid Import (Wh)", color="blue")
# axes[1].set_ylabel("Energy (Wh)")
# axes[1].legend(bbox_to_anchor=(1, 1), loc='upper right')
# axes[1].grid(True)

# df["Grid_export"].plot(ax=axes[2], label="Grid Export (Wh)", color="yellow")
# (-df["Grid_import"]).plot(ax=axes[2], label="Grid Import (W)", color="blue")
# axes[2].set_ylabel("Energy (Wh)")
# axes[2].legend(bbox_to_anchor=(1, 1), loc='upper right')
# axes[2].grid(True)

# df["Battery_charge"].plot(ax=axes[3], label="Battery Charge (Wh)", color="orange")
# (-df["Battery_discharge"]).plot(ax=axes[3], label="Battery Discharge (Wh)", color="purple")
