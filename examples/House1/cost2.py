...
energy_summaries = os.path.join(script_dir, "output","energy_summaries")
...
def calculate_installation_cost(kwh_capacity):
    if kwh_capacity == 0:
        return 0
    cap_kWh = kwh_capacity / 1000.0   # Wh → kWh
...
# --- NEW: Dynamically build installation_costs dictionary ---
installation_costs = {
...
for scenario_name, config in scenarios_config.items():
    ...
    full_csv_path = os.path.join(output_directory,energy_summaries, csv_filename)
