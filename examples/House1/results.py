...
results = pd.read_csv(os.path.join("flows_nobattery", "flow_NB_apr23.csv"), header=[0, 1], index_col=0)
...
start_date = pd.to_datetime('2023-04-01', utc=True).date()
end_date = pd.to_datetime('2023-04-30', utc=True).date()
delta = pd.Timedelta(days=5)
pdf_path = os.path.join("pdf", "nb_apr23.pdf")
...
with PdfPages(pdf_path) as pdf:
    ...
    while current_date <= end_date:
        ...
        if not df_filtered.empty:
            fig, axes = plt.subplots(nrows=2, ncols=1, figsize=(14, 20), sharex=True)
