...
input_csv_path = os.path.join(script_dir, "output", "scenario_net_costs1.csv")
# Directory where individual monthly energy summary CSVs are located
energy_summary_dir = os.path.join(script_dir, "output", "energy_summaries")
...
# --- MODIFIED: Helper function to get annual import/export from MONTHLY files ---
def get_annual_energy_data(scenario_name, monthly_files_directory):
    if scenario_name == 'NoBattery':
        filename = f"monthly_energy_summary_2023_nobattery_kWh.csv"
    else:
        # Convert '10kWh' to '10k' for filename matching
        battery_size_for_filename = scenario_name.replace('kWh', 'k')
        filename = f"monthly_energy_summary_2023_battery_{battery_size_for_filename}_kWh.csv"

    filepath = os.path.join(monthly_files_directory, filename)

    if not os.path.exists(filepath):
        print(f"Warning: Monthly energy summary file not found for {scenario_name} at {filepath}. Using 0 for import/export.")
        return 0.0, 0.0 # Return zeros if file not found

    try:
        monthly_df = pd.read_csv(filepath)
        
        # Sum up the monthly grid import and export values to get the annual total
        grid_import_total = monthly_df['Grid Import (kWh)'].sum()
        grid_export_total = monthly_df['Grid Export (kWh)'].sum()
        
        return grid_import_total, grid_export_total
    except KeyError as e:
        print(f"Error: Required column {e} not found in {filepath}. Using 0 for import/export.")
        return 0.0, 0.0
    except Exception as e:
        print(f"Error reading monthly energy data from {filepath} for {scenario_name}: {e}. Using 0 for import/export.")
        return 0.0, 0.0
