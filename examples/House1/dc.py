...
# --- NEW Helper function to plot combined yearly SOC duration curve ---
def export_combined_yearly_soc_duration_curve(soc_data_for_scenarios, output_dir):
    """
    Generates and saves a single plot with yearly SOC duration curves for all specified scenarios.
    soc_data_for_scenarios is a dictionary where keys are scenario names and values are DataFrames
    containing "SOC_%" and "Month" columns for the entire year.
    """
    combined_pdf_path = os.path.join(output_dir, "Combined_Yearly_SOC_Duration_Curves.pdf")
    combined_png_path = os.path.join(output_dir, "Combined_Yearly_SOC_Duration_Curves.png")

    print("\n--- Generating Combined Yearly SOC Duration Curve for all scenarios ---")

    fig, ax = plt.subplots(figsize=(12, 12 * 13 / 21))

    for scenario_name, yearly_df in soc_data_for_scenarios.items():
        # Ensure the DataFrame has the 'SOC_%' column and it's not all NaNs
        if "SOC_%" in yearly_df.columns and not yearly_df["SOC_%"].isnull().all():
            sorted_soc_data = yearly_df.sort_values("SOC_%", ascending=False).reset_index(drop=True)
            duration_hours = np.arange(1, len(sorted_soc_data) + 1) / 60.0
            ax.plot(duration_hours, sorted_soc_data["SOC_%"], label=scenario_name)
        else:
            print(f"    Skipping {scenario_name} for combined plot: No valid SOC data found.")

    ax.set_xlabel("Duration (Hours)")
    ax.set_ylabel("SOC (%)")
    ax.set_title("Combined Yearly SOC Duration Curves for All Scenarios")
    ax.grid(True)
    ax.set_ylim(0, 100)
    ...
    with PdfPages(combined_pdf_path) as pdf:
        pdf.savefig(fig)
    print(f"Combined yearly SOC duration curve saved to: {combined_pdf_path}")

    # Save to PNG
    fig.savefig(combined_png_path, dpi=300)
    print(f"Combined yearly SOC duration curve saved to: {combined_png_path}")
...
def process_scenario_data(scenario_name, flow_dir, file_dict, capacity):
    ...
    if not all_monthly_dfs:
        ...
        return None, None # Return None for yearly_df if no data processed
    ...
    if f"SOC_{scenario_name}_%" in yearly_df.columns and not yearly_df[f"SOC_{scenario_name}_%"].isnull().all():
        ...
        yearly_soc_for_combined_plot = soc_for_duration_curves # Return this for the combined plot
    else:
        print(f"    Skipping SOC duration curves for {scenario_name}: No valid SOC data found.")
        yearly_soc_for_combined_plot = None
    ...
    return yearly_df, yearly_soc_for_combined_plot # Return both the full yearly_df and the SOC-specific one
...
# Updated scenarios_config with the new 50kWh battery
scenarios_config = {
    "5kWh": {
    ...
    # --- NEW SCENARIO ---
    "50kWh": { 
        "flow_dir": os.path.join(script_dir, "flows_50k"), # Assuming a new folder named 'flows_50k' for this capacity
        "files": {
        ...
        "capacity": 50000 # 50 kWh = 50000 Wh
    }
}
...
if __name__ == "__main__":
    ...
    yearly_soc_data_for_combined_plot = {} # Dictionary to store yearly SOC data for combined plot

    for name, config in scenarios_config.items():
        if config["capacity"] > 0:
            yearly_df, soc_for_combined = process_scenario_data(name, config["flow_dir"], config["files"], config["capacity"])
            if soc_for_combined is not None:
                yearly_soc_data_for_combined_plot[name] = soc_for_combined
    ...
    # Generate the combined yearly SOC duration curve
    if yearly_soc_data_for_combined_plot:
        output_base_dir = os.path.join(script_dir, "output")
        export_combined_yearly_soc_duration_curve(yearly_soc_data_for_combined_plot, output_base_dir)
    else:
        print("\nNo battery scenarios found with valid SOC data for combined yearly duration curve.")
