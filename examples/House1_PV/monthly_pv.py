...
def calculate_monthly_flows(scenario_identifier, scenario_config):
    """
    ...
    for month, filename in scenario_config["files"].items():
        ...
        try:
            ...
            monthly_flows = {"Scenario": scenario_identifier, "Month": month}
            
            # Loop through all the specified flows and sum them up for the month
            for flow_name, solph_label_tuples in ALL_FLOWS.items():
                monthly_total_wh = 0.0
                ...
                monthly_flows[flow_name] = monthly_total_wh / 60000
                
            monthly_results.append(monthly_flows)
...
if __name__ == "__main__":
    ...
    # Scenarios config from the user's previous script
    scenarios_config = {
        "PV_NoBattery": {
    ...
    all_monthly_flows_data = []

    for scenario_identifier, config in scenarios_config.items():
        print(f"\n--- Processing scenario: {scenario_identifier} ---")
        monthly_flows_list = calculate_monthly_flows(scenario_identifier, config)
        if monthly_flows_list:
            all_monthly_flows_data.extend(monthly_flows_list)
    
    if all_monthly_flows_data:
        df_monthly_flows_master = pd.DataFrame(all_monthly_flows_data)
        
        output_filepath = os.path.join(output_dir, "monthly_energy_flows_master_table.csv")
