...
# The input directory is updated to match the previous script's output
...
def order_dataframe_for_plotting(df):
    """Orders a DataFrame based on the new scenario order."""
    # Updated order to match the new scenarios from previous steps
    ...
    return df.set_index("Scenario").reindex(order).reset_index()
...
# Order the DataFrame before plotting
df_ordered = order_dataframe_for_plotting(df)
...
fig, ax = plt.subplots(figsize=(14, 7))
bar = ax.bar(df_ordered["Scenario"], df_ordered["Operational Breakeven Time (hours)"], color=sns.color_palette("magma", len(df_ordered)))
...
fig, ax = plt.subplots(figsize=(14, 7))
bar = ax.bar(df_ordered["Scenario"], df_ordered["Energy Breakeven (kWh)"], color=sns.color_palette("plasma", len(df_ordered)))
...
fig, ax = plt.subplots(figsize=(14, 7))
sns.scatterplot(data=df_ordered, x="Breakeven_With_Setup_Years", y="Carbon Breakeven Time (years)", hue="Scenario", s=100, ax=ax, palette="viridis")
