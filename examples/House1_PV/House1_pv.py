...
op_data_full = pd.read_csv(os.path.join("op_data_power_pv.csv"))
...
for month_info in months_to_simulate:
    ...
    monthly_House1.add(demands.Electricity(name="demand", time_series=op_data["Load_Watts"]))
    monthly_House1.add(technologies.RenewableElectricitySource(
        name="PV",
        nominal_power=1,
        specific_generation=op_data["Modelled_power1"],
        fixed=True
    ))
    monthly_House1.add(technologies.RenewableElectricitySource(
        name="PV2",
        nominal_power=1,
        specific_generation=op_data["Modelled_power2"],
        fixed=True
    ))
    ...
    plot.render(outfile="House1_pv1.png")
